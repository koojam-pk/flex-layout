...
export class StyleService {
  ...
  constructor(private _serverStylesheet: ServerStylesheet,
              ...
              @Optional() @Inject(DOCUMENT) _document,
  ...
  lookupStyle(element: HTMLElement, styleName: string, inlineOnly = false): string {
    ...
    if (element) {
      let immediateValue = value = this.lookupInlineStyle(element, styleName) ||
        this._attributeToObject(this.lookupAttributeValue(element, 'style'))[styleName];
      if (!immediateValue) {
   ...
   *
   */
  private _attributeToObject(attString: string) {
    const attList = attString.split(ATT_DELIMITER);
    const attObj = {};

    attList.forEach(att => {
      const [key, value] = att.split(PROP_DELIMITER, 2);
      attObj[key] = value;
    });
